...
void CPlaneGraphic::LoadCurveData()
{
	...
	strsql.Format("select * FROM  Scheme_plane_CureveData WHERE 方案名称='%s' ORDER BY 交点序号 ASC",m_Schemename);	
	try
	{
	...
	strsql.Format("select * FROM  Scheme_plane_CureveData_XY WHERE 方案名称='%s' ORDER BY 交点序号 ASC",m_Schemename);	
	try
	{
		m_Recordset->Open(_bstr_t(strsql),(IDispatch*)(theApp.m_pConnection),adOpenDynamic,adLockOptimistic,adCmdText);    
	}
...
void CPlaneGraphic::OnMenuPlaneOutcad() 
{
	...
	{
		this->MessageBox("文件"+DxfFilename+"已被其它程序打开或占用,请关闭再试!","输出到CAD",MB_ICONINFORMATION);
		return;
	}
	...
	for(int i=0;i<ployLineCAD.GetSize();i++)
	{
		mdxf.DxfStart_polyline(Dxffile,"图层polygon",0,m_penWidth[ployLineCAD.GetAt(i)->Index]-1,\
	...
	for(i=0;i<TextCAD.GetSize();i++)
	{
		mdxf.DxfDraw_Text(Dxffile,"标注",TextCAD.GetAt(i)->x,TextCAD.GetAt(i)->y,\
	...
	MessageBox(tt,"输出图形到AuotCAD",MB_ICONINFORMATION);	
}
