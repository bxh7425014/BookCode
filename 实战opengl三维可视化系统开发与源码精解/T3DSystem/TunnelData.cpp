...
void CTunnelData::InitGrid()
{
	...
	m_grid.SetTextMatrix(0,1,"隧道名称");
	m_grid.SetTextMatrix(0,2,"起点里程");
	m_grid.SetTextMatrix(0,3,"终点里程");
	m_grid.SetTextMatrix(0,4,"隧道长度");
...
BOOL CTunnelData::CheckValid(BOOL ADDorEDit)
{
	...
	if(ADDorEDit==TRUE) 
		tt.Format("SELECT *  FROM  Tunnel WHERE 方案名称='%s' AND \
		隧道名称='%s'",myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename,m_name);
	else
		tt.Format("SELECT *  FROM  Tunnel WHERE 方案名称='%s' AND \
		  隧道名称='%s' AND 隧道名称!='%s'" ,myDesingScheme.SchemeDatass[m_currentSchemeIndexs].Schemename,m_name,m_oldname);
...
void CTunnelData::OnButtonEdit() 
{
	...
	tt.Format("DELETE FROM  Tunnel WHERE 隧道名称='%s' AND 方案名称='%s'",\
...
void CTunnelData::WriteToExcel()
{
	...
	try
	{
		...
		sSql = "CREATE TABLE "+sTableFile+" (隧道名称 TEXT,隧道起点里程  NUMBER,\
			隧道终点里程 NUMBER,隧道长度 NUMBER)";
