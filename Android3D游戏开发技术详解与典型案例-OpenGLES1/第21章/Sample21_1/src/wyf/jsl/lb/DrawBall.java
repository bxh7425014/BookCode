...
public class DrawBall {   
    ...
    public DrawBall(float scale,int texId,float aspan) 
    {
    	this.texId=texId;
    	this.scale=scale;    	
        ...
        for(float vAngle=90;vAngle>-90;vAngle=vAngle-aspan)//垂直方向angleSpan度一份
        {
        	for(float hAngle=360;hAngle>0;hAngle=hAngle-aspan)//水平方向angleSpan度一份
        	{
        		...
        		xozLength=scale*UNIT_SIZE*Math.cos(Math.toRadians(vAngle-aspan));
        		float x2=(float)(xozLength*Math.cos(Math.toRadians(hAngle)));
        		float z2=(float)(xozLength*Math.sin(Math.toRadians(hAngle)));
        		float y2=(float)(scale*UNIT_SIZE*Math.sin(Math.toRadians(vAngle-aspan)));
        		...
        		float y3=(float)(scale*UNIT_SIZE*Math.sin(Math.toRadians(vAngle-aspan)));
