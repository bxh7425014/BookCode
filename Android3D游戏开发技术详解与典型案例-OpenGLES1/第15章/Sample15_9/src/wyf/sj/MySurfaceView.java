...
public class MySurfaceView extends GLSurfaceView {
	...
	private class SceneRenderer implements GLSurfaceView.Renderer 
    {
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            ...
            skyId=initTexture(gl,R.drawable.skyball);
            //创建地形
            landform=
            	new LandForm(grassTextureId,yArray,yArray.length-1,yArray[0].length-1);
            sky=new Sky_Ball(BALL_R,skyId);
	...
	{
		float[] fogColor={0.96470f,0.96862f,0.91372f,0};//雾的颜色
		gl.glFogfv(GL10.GL_FOG_COLOR, fogColor, 0);//设置雾的颜色
		gl.glFogx(GL10.GL_FOG_MODE, GL10.GL_EXP2);//设置雾的模式
		gl.glFogf(GL10.GL_FOG_DENSITY, 0.006f);//设置雾的浓度
		gl.glFogf(GL10.GL_FOG_START, BALL_R-FOG_D);//设置雾的开始距离
		gl.glFogf(GL10.GL_FOG_END, BALL_R);//设置雾的结束距离
	}
	//初始化纹理
